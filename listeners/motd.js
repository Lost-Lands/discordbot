module.exports = (bot) => {
    bot.on("motd", (message, content) => {
        ...
        if (!message.member.roles.cache.some((role) => role.id === bot.config.roles.verified_player)) {
            ...
            if (motd.length > 5 && motd.length < 46) { //check if MOTD suggestion is too long
                ...
                return message.reply("MOTD must be between 5 and 45 characters. Your MOTD suggestion was "+motd.length);
            }
        }
    });
}